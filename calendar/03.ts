...
const alphabetUpper = alphabet.map((x) => x.toUpperCase());

const arr = input.split("\n");

const part1 = arr
  .map(
    (x) => [x.slice(0, x.length / 2), x.slice(x.length / 2, x.length)] as const
  )
  .map(([first, second]) =>
    first.split("").find((char) => second.includes(char))
  )
  .filter((x): x is string => !!x)
  .map((x) =>
    x.toLowerCase() === x
      ? alphabet.indexOf(x) + 1
      : alphabetUpper.indexOf(x) + 27
  )
